-- migration_v16: Community tables + Course Enrollment & Access Control
-- Safe to run on a fresh DB (creates v15 tables if missing) or on top of v15.
-- Adds access_type / auto_enroll_on_join to community_modules and a new
-- community_module_enrollments table. Updates get_community_modules RPC.

-- ── Community tables (idempotent — create if not already present) ──

CREATE TABLE IF NOT EXISTS community_posts (
  id               uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
  org_id           uuid        NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  author_type      text        NOT NULL DEFAULT 'coach',
...
CREATE TABLE IF NOT EXISTS community_modules (
  ...
  access_type          text        NOT NULL DEFAULT 'all' CHECK (access_type IN ('all','enrolled')),
  auto_enroll_on_join  boolean     NOT NULL DEFAULT false,
  created_at           timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS community_lesson_progress (
  id           uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- ── Add new columns to community_modules (safe if already exist) ──
...
CREATE INDEX IF NOT EXISTS idx_community_posts_org        ON community_posts(org_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_community_modules_org      ON community_modules(org_id, position ASC);
CREATE INDEX IF NOT EXISTS idx_community_lessons_module   ON community_lessons(module_id, position ASC);
CREATE INDEX IF NOT EXISTS idx_community_progress_client  ON community_lesson_progress(client_id);
CREATE INDEX IF NOT EXISTS idx_community_enrollments_module ON community_module_enrollments(module_id);
CREATE INDEX IF NOT EXISTS idx_community_enrollments_client ON community_module_enrollments(client_id);
...
DROP POLICY IF EXISTS "coach_community_posts"       ON community_posts;
DROP POLICY IF EXISTS "coach_community_reactions"   ON community_reactions;
DROP POLICY IF EXISTS "coach_community_comments"    ON community_comments;
DROP POLICY IF EXISTS "coach_community_modules"     ON community_modules;
DROP POLICY IF EXISTS "coach_community_lessons"     ON community_lessons;
DROP POLICY IF EXISTS "coach_community_progress"    ON community_lesson_progress;
DROP POLICY IF EXISTS "coach_community_enrollments" ON community_module_enrollments;
...
CREATE OR REPLACE FUNCTION public.add_community_comment(p_client_id uuid, p_post_id uuid, p_content text)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE v_org_id uuid; v_name text; v_id uuid; v_ts timestamptz; BEGIN
  ...
  RETURN jsonb_build_object('id', v_id, 'content', p_content, 'author_name', v_name, 'author_type', 'client', 'created_at', v_ts);
END; $$;
...
-- Updated: respects access_type + enrollment
